...
- **Unified:** feedback (PID), sampling-MPC (MPPI/CEM/ICEM), gradient-based
  MPC (ILQR), safety (CBF), and RL (PPO on-policy; SAC/TD3 off-policy; GRPO —
  a critic-free policy gradient that scores grouped action samples from a
  branchable state instead) all share ``predict``/``learn``/``save``/``load``.
